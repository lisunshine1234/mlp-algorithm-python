import numpy as np
from sklearn.decomposition import DictionaryLearning, FastICA, FactorAnalysis, IncrementalPCA, KernelPCA, \
    LatentDirichletAllocation, MiniBatchDictionaryLearning, MiniBatchSparsePCA, NMF, PCA, SparseCoder, SparsePCA, \
    TruncatedSVD
...
def run(array, label_index, n_components, copy, whiten, svd_solver, tol, iterated_power, random_state):
    ...
    if type(iterated_power) is str and iterated_power != 'auto':
        feature_range = eval(iterated_power)
