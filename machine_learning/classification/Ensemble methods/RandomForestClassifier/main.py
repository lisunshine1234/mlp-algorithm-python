...
criterion	标准	默认为'gini',衡量分割质量的功能。支持的标准是：基尼杂质'gini'和信息增益'entropy'。注意：此参数是特定于树的,可选'entropy','gini'	字符串	不必须	定数
...
min_samples_leaf	最小样本数	默认为1,在叶节点处所需的最小样本数。仅在任何深度的分裂点在左分支和右分支中的每个分支上至少留下'min_samples_leaf'个训练样本时，才考虑。-如果为int，则将'min_samples_leaf'作为最小数字。-如果为float，则'min_samples_leaf'是分数，而'ceil(min_samples_leaf * n_samples)'是每个节点的最小样本数,可选整数,浮点数	字符串	不必须	定数
min_weight_fraction_leaf	最小重量分数叶子	默认为0.,在所有叶节点处(所有输入样本)的权重总和中的最小加权分数。如果未提供sample_weight，则样本的权重相等,可选浮点数	浮点数	不必须	定数
max_features	最佳分割特征数量	默认为'auto',寻找最佳分割时要考虑的要素数量：。-如果为int，则在每个分割中考虑'max_features'个要素。-如果为float，则'max_features'为小数，并且在每个拆分中均考虑'int(max_features * n_features)'特征。-如果是'auto'，则为'max_features=sqrt(n_features)'。-如果是'sqrt'，则'max_features=sqrt(n_features)'(与'auto'相同)。-如果是'log2'，则为'max_features=log2(n_features)'。-如果为None，则'max_features=n_features'。注意：在找到至少一个有效的节点样本分区之前，分割的搜索不会停止，即使它需要有效地检查'max_features'个以上的特征,可选整数,浮点数,'sqrt','auto','log2'	字符串	不必须	定数
...
bootstrap	bootstrap	默认为True,建立树木时是否使用引导程序样本。如果为False，则将整个数据集用于构建每棵树,可选布尔值,字符串	字符串	不必须	定数
...
random_state	随机种子	默认为None,控制构建树时使用的示例的自举的随机性(如果是'bootstrap=True')和在每个节点上寻找最佳分割时要考虑的要素采样(如果是'max_features < n_features'),可选整数	整数	不必须	定数
...
class_weight	类别权重	默认为None,与类关联的权重，格式为'{class_label: weight}'。如果未给出，则所有类都应具有权重一。对于多输出问题，可以按与y列相同的顺序提供字典列表。请注意，对于多输出(包括多标签)，应在其自己的字典中为每列的每个类定义权重。 'balanced'模式使用y的值自动将与输入数据中的类频率成反比的权重调整为'n_samples / (n_classes * np.bincount(y))'。'balanced_subsample'模式与'balanced'相同，不同之处在于，权重是根据每个生长的树的引导程序样本计算的。对于多输出，y的每一列的权重将相乘。请注意，如果指定了sample_weight，则这些权重将与sample_weight(通过fit方法传递)相乘,可选list,字典,'balanced','balanced_subsample'	字符串	不必须	定数
...
feature_importances_	特征重要性	基于杂质的功能的重要性。越高，功能越重要。特征的重要性计算为该特征带来的标准的(标准化)总缩减。这也被称为基尼重要性。警告：基于杂质的特征重要性可能会误导高基数特征(许多唯一值)。参见：func：'sklearn.inspection.permutation_importance'作为替代	一维数组(数值)
...
[outline]
随机森林分类器。
随机森林是一种元估计量，它适合数据集各个子样本上的许多决策树分类器，并使用平均数来提高预测准确性和控制过度拟合。
如果bootstrap = True(默认值)，则子样本大小由max_samples参数控制，否则，整个数据集将用于构建每棵树。
...
def main(x_train, y_train, x_test, y_test,
         ...
         max_leaf_nodes=None, min_impurity_decrease=0., min_impurity_split=None, bootstrap=True, oob_score=False, n_jobs=None, random_state=None, verbose=0,
    ...
    if type(class_weight) is str and class_weight != 'balanced' and class_weight != 'balanced_subsample':
    ...
    return r.run(x_train=x_train, y_train=y_train, x_test=x_test, y_test=y_test, n_estimators=n_estimators,
                 criterion=criterion,
                 max_depth=max_depth,
