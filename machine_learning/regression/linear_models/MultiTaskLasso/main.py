...
coef_	参数向量	参数向量(成本函数公式中的W)。注意coef_存储W，W.T的转置	二维数组
intercept_	截距	决策函数中的独立项	整数
n_iter_	迭代次数	坐标下降求解器运行以达到指定公差的迭代次数	整数

[outline]
以L1/L2混合范数为正则训练的多任务套索模型。

[describe]
以L1/L2混合范数为正则训练的多任务套索模型。
套索的优化目标是：(1/(2*n_samples))*||Y-XW||^2_Fro+alpha*||W||_21哪里：：||W||_21=\\sum_i\\sqrt{\\sum_jw_{ij}^2}即每行范数的总和
'''


def main(x_train, y_train, x_test, y_test,
         alpha=1.0, fit_intercept=True, normalize=False, copy_X=True, max_iter=1000, tol=1e-4, warm_start=False,
