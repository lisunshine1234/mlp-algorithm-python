...
alpha	alpha	默认为1.0,乘以惩罚项的常数。默认为1.0。有关此参数的确切数学含义，请参见注释。alpha=0等效于一个普通的最小二乘，由LineclassRegression对象解决。出于数字原因，不建议将alpha=0与Lasso对象一起使用,可选浮点数	浮点数	不必须	定数
l1_ratio	l1_ratio	默认为0.5,ElasticNet混合参数，其值为0<=l1_ratio<=1。当l1_ratio=0时，惩罚为L2惩罚。对于l1_ratio=1，这是L1惩罚。对于0<l1_ratio<1，惩罚是L1和L2的组合,可选浮点数	浮点数	不必须	定数
...
precompute	预先计算	默认为False,default=False是否使用预先计算的Gram矩阵来加快计算速度。语法矩阵也可以作为参数传递。对于稀疏输入，此选项始终为True以保留稀疏性,可选布尔值,数组	字符串	不必须	定数
...
sparse_coef_	稀疏参数向量	(n_targets，n_features)sparse_coef_是一个衍生自coef_的只读属性	二维数组
...
[describe]
将L1和L2先验组合作为正则化器的线性回归。
最小化目标函数：1/(2*n_samples)*||y-Xw||^2_2+alpha*l1_ratio*||w||_1+0.5*alpha*(1-l1_ratio)*||w||^2_2如果您有兴趣控制L1和L2罚款请记住，这等效于：a*L1+b*L2哪里：：alpha=a+b和l1_ratio=a/(a+b)参数l1_ratio对应于glmnetR包中的alpha，而alpha对应于glmnet中的lambda参数。
具体来说，l1_ratio=1是套索惩罚。
目前，l1_ratio<=0.01不可靠，除非您提供自己的alpha序列
'''
