...
[describe]
具有l2正则化的线性最小二乘法。
最小化目标函数：||	y-Xw	||	^	2_2	+	alpha	*	||	w	||	^	2_2此模型求解回归模型，
其中损失函数为线性最小二乘函数，
而正则化由l2-范数给出。也称为Ridge回归或Tikhonov正则化。
此估算器具有对多元回归的内置支持（即，当y是形状的2d数组（n_samples，n_targets）时）。
'''


def main(x_train, y_train, x_test, y_test, alpha=1.0, fit_intercept=True, normalize=False, copy_X=True, max_iter=None, tol=1e-3, solver="auto",
         random_state=None):
