...
radius	radius	默认为1.0,默认用于：meth：'radius_neighbors'查询的参数空间范围,可选浮点数	浮点数	不必须	定数
...
metric	树的距离度量	默认为minkowski,用于树的距离度量。默认度量标准为minkowski，p = 2等效于标准欧几里德度量标准。有关可用度量的列表，请参见：class：'DistanceMetric'的文档。如果度量为'precomputed'，则假定X为距离矩阵，并且在拟合过程中必须为正方形。 X可以是：term：'sparse graph'，在这种情况下，只有'nonzero'元素可以被视为邻居,可选'minkowski'	字符串	不必须	定数
metric_params	度量的其他关键字参数	默认为None,度量特征的其他关键字参数,可选字典	字典	不必须	定数
...
def main(x_train, y_train, x_test, y_test,
         radius=1.0, weights='uniform', algorithm='auto', leaf_size=30, p=2, metric='minkowski', metric_params=None, n_jobs=None
         , **kwargs):
