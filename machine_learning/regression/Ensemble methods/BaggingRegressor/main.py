...
warm_start	warm_start	默认为False,设置为True时，请重用上一个调用的解决方案以进行拟合，并向集合添加更多估计量，否则，就可以拟合,可选布尔值	布尔值	不必须	定数
...
oob_prediction_	oob_prediction_	使用训练集上的实际估计值来计算预测。如果n_estimators小，则有可能在引导过程中从未遗漏任何数据点。在这种情况下，'oob_prediction_'可能包含NaN。仅当'oob_score'为True时，此属性才存在	一维数组(数值)

[outline]
套袋回归器。
Bagging回归器是一个集合元估计器，它使每个基本回归器都适合原始数据集的随机子集，然后将其单个预测(通过投票或平均)进行汇总以形成最终预测。
通过将随机化引入其构造过程中，然后对其进行整体化，这样的元估计器通常可以用作减少黑盒估计器(例如，决策树)的方差的方式。
该算法涵盖了文献中的几篇著作。
当将数据集的随机子集绘制为样本的随机子集时，该算法称为粘贴[1] _。
...
def main(x_train, y_train, x_test, y_test,
         base_estimator=None, estimator_params=None, n_estimators=10, max_samples=1.0, max_features=1.0, bootstrap=True, bootstrap_features=False,
