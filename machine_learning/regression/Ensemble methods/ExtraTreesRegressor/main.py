...
criterion	标准	默认为'mse',衡量分割质量的功能。支持的标准是用于均方误差的'mse'(等于作为特征选择标准的方差减少)和用于均方绝对误差的'mae',可选'mse','mae'	字符串	不必须	定数
...
max_features	最佳分割特征数量	默认为'auto',寻找最佳分割时要考虑的要素数量：。-如果为int，则在每个分割中考虑'max_features'个要素。-如果为float，则'max_features'为小数，并且在每个拆分中均考虑'int(max_features * n_features)'特征。-如果是'auto'，则为'max_features=n_features'。-如果是'sqrt'，则为'max_features=sqrt(n_features)'。-如果是'log2'，则为'max_features=log2(n_features)'。-如果为None，则'max_features=n_features'。注意：在找到至少一个有效的节点样本分区之前，分割的搜索不会停止，即使它需要有效地检查'max_features'个以上的特征,可选整数,浮点数,'auto','log2','sqrt'	字符串	不必须	定数
...
n_features_	特征数量	功能数量	整数
n_outputs_	输出数量	输出数量	整数
