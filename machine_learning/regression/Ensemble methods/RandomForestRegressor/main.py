...
criterion	标准	默认为'mse',衡量分割质量的功能。支持的标准是用于均方误差的'mse'(等于作为特征选择标准的方差减少)和用于均方绝对误差的'mae',可选'mae','mse'	字符串	不必须	定数
...
max_features	最佳分割特征数量	默认为'auto',寻找最佳分割时要考虑的要素数量：。-如果为int，则在每个分割中考虑'max_features'个要素。 。-如果为float，则'max_features'为小数，并且在每个拆分中均考虑'int(max_features * n_features)'特征。 。-如果为'auto'，则为'max_features=n_features'。 。-如果为'sqrt'，则为'max_features=sqrt(n_features)'。 。-如果为'log2'，则为'max_features=log2(n_features)'。 。-如果为None，则'max_features=n_features'。注意：在找到至少一个有效的节点样本分区之前，分割的搜索不会停止，即使它需要有效地检查'max_features'个以上的特征,可选整数,浮点数,'sqrt','auto','log2'	字符串	不必须	定数
...
oob_score	oob_score	默认为False,是否使用袋外样本估算看不见的数据的R ^ 2,可选布尔值	布尔值	不必须	定数
...
oob_prediction_	oob_prediction_	使用训练集上的实际估计值来计算预测。仅当'oob_score'为True时，此属性才存在	一维数组(数值)

[outline]
随机森林回归器。
随机森林是一种元估计量，它适合数据集各个子样本上的许多分类决策树，并使用平均数来提高预测准确性和控制过度拟合。
如果'bootstrap = True'(默认)，则用'max_samples'参数控制子样本的大小，否则整个数据集将用于构建每棵树。
...
def main(x_train, y_train, x_test, y_test,
         n_estimators=100, criterion="mse", max_depth=None, min_samples_split=2, min_samples_leaf=1, min_weight_fraction_leaf=0., max_features="auto",
